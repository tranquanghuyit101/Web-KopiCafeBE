...
class AiSuggestServiceImplTest {
    ...
    void setUp() {
        ...
        // default region đã có @Value nhưng không ảnh hưởng tới test vì mình truyền tham số cụ thể vào yt.*
    }
    ...
    void should_GroupRankAndAnnotateVideos_When_GeminiAndYouTubeReturnData() {
        // Given
        var req = AiSuggestionRequest.builder().days(30).maxResults(10).build();
        ...
        when(yt.searchRecentVideos(eq("Cà phê muối"), anyInt(), anyInt(), anyString(), anyString(), anyBoolean(), isNull(), anyString(), anyBoolean()))
                .thenReturn(List.of(
                        video("v1", "Cách làm Cà phê muối ngon", "desc", 20000, 1000, now.minusDays(1)),
                        video("v2", "Cách pha Cà phê muối chuẩn", "desc", 15000, 800, now.minusDays(2))
                ));
        when(yt.searchRecentVideos(eq("Matcha latte"), anyInt(), anyInt(), anyString(), anyString(), anyBoolean(), isNull(), anyString(), anyBoolean()))
                .thenReturn(List.of(
                        video("v3", "Cách làm Matcha latte béo", "desc", 18000, 900, now.minusDays(1)),
                        video("v4", "How to make matcha latte", "desc", 16000, 700, now.minusDays(3))
                ));
        ...
        // sắp xếp theo topScore giảm dần (khó xác định tuyệt đối, nhưng kiểm tra có topScore>0 và videos sorted theo viralScore)
        for (DishGroup g : res.getData()) {
            ...
            for (int i = 1; i < list.size(); i++) {
                assertThat(list.get(i-1).getViralScore()).isGreaterThanOrEqualTo(list.get(i).getViralScore());
            }
        }
    ...
    void should_UseFallbackDishes_When_GeminiThrows_AndReturnGroupsEvenWithNoVideos() {
        // Given
        var req = AiSuggestionRequest.builder().days(15).maxResults(5).build();
        ...
        // YouTube rỗng
        when(yt.searchRecentVideos(anyString(), anyInt(), anyInt(), anyString(), anyString(), anyBoolean(), any(), anyString(), anyBoolean()))
                .thenReturn(List.of());
        when(yt.searchVideos(anyString(), anyInt(), anyInt(), anyString(), anyString(), anyBoolean(), any(), anyString(), anyBoolean(), anyString()))
                .thenReturn(List.of());
    ...
    void should_FilterOutIrrelevantOrForeignLanguageVideos() {
        // Given
        var req = AiSuggestionRequest.builder().days(30).maxResults(5).build();
        var d = dishInfo("Trà sữa", 6.0, "R");
        when(gemini.findHotTrendDishes(anyInt(), anyInt())).thenReturn(List.of(d));

        var now = OffsetDateTime.now();
        var jp = video("x1", "抹茶ラテ 作り方 (matcha latte)", "Japanese title", 5000, 200, now.minusDays(1)); // chứa kana → bị reject
        var ok = video("x2", "Cách làm Trà sữa trân châu tại nhà", "recipe", 10000, 500, now.minusDays(1));
        when(yt.searchRecentVideos(eq("Trà sữa"), anyInt(), anyInt(), anyString(), anyString(), anyBoolean(), any(), anyString(), anyBoolean()))
    ...
    void should_PopulateMetaFields_And_SortVideosByViralScoreDesc() {
        // Given
        var req = AiSuggestionRequest.builder().days(7).maxResults(6).build();
        var d = dishInfo("Cà phê trứng", 7.0, "R");
        when(gemini.findHotTrendDishes(anyInt(), anyInt())).thenReturn(List.of(d));

        var now = OffsetDateTime.now();
        var v1 = video("a", "Cách làm Cà phê trứng", "desc", 30000, 1500, now.minusHours(12));
        var v2 = video("b", "Cách làm Cà phê trứng chuẩn", "desc", 10000, 200, now.minusHours(2)); // ít view/like nhưng rất mới
        when(yt.searchRecentVideos(eq("Cà phê trứng"), anyInt(), anyInt(), anyString(), anyString(), anyBoolean(), any(), anyString(), anyBoolean()))
        ...
        assertThat(g.getVideos().get(0).getViralScore()).isGreaterThanOrEqualTo(g.getVideos().get(1).getViralScore());
    }
    ...
    void should_MapDishGroupsToSuggestedDrinks_InSearch() {
        ...
        List<VideoItem> vids = List.of(
                video("v1", "Cách làm Dirty Matcha", "desc", 1000, 50, OffsetDateTime.now().minusDays(1))
        );
    ...
    void should_ReturnEmptyItems_When_GroupedByDishReturnsEmpty() {
        // Given
        var req = AiSuggestionRequest.builder().build();
        AiSuggestServiceImpl spyService = Mockito.spy(new AiSuggestServiceImpl(yt, gemini));
    ...
    void should_KeepAllGroups_AsItems_InSearch() {
        ...
        DishGroup g1 = DishGroup.builder().key("a").name("A").topScore(70.0).videos(List.of()).totalVideos(0).build();
        DishGroup g2 = DishGroup.builder().key("b").name("B").topScore(80.0).videos(List.of()).totalVideos(0).build();
        DishTrendsResponse trends = DishTrendsResponse.builder()
                .data(List.of(g1, g2))
        ...
        assertThat(resp.getItems()).extracting(AiSuggestResponse.SuggestedDrink::getName)
                .containsExactly("A", "B");
    }
    ...
    void should_FillDefaultTags_AndModel_InSearch() {
        ...
        var req = AiSuggestionRequest.builder().build();
        AiSuggestServiceImpl spyService = Mockito.spy(new AiSuggestServiceImpl(yt, gemini));
        DishGroup g = DishGroup.builder().key("x").name("X").topScore(10.0).videos(List.of()).totalVideos(0).build();
        DishTrendsResponse trends = DishTrendsResponse.builder()
                .data(List.of(g))
