...
class PromoServiceImplTest {
    ...
    private DiscountCode dc(Integer id, String code, String desc,
        ...
        d.setCode(code);
        d.setDescription(desc == null ? "" : desc);
        d.setActive(active == null ? Boolean.FALSE : active);
        d.setStartsAt(starts == null ? LocalDateTime.now().minusDays(1) : starts);
        d.setEndsAt(ends == null ? LocalDateTime.now().plusDays(1) : ends);
        d.setCreatedAt(created == null ? LocalDateTime.now() : created);
        d.setDiscountValue(value == null ? BigDecimal.ZERO : value);
        d.setDiscountType(DiscountType.PERCENT);
        return d;
    }
    ...
    private DiscountEvent ev(Integer id, String name, String desc,
        ...
        e.setActive(active == null ? Boolean.FALSE : active);
        e.setStartsAt(starts == null ? LocalDateTime.now().minusDays(1) : starts);
        e.setEndsAt(ends == null ? LocalDateTime.now().plusDays(1) : ends);
        e.setCreatedAt(created == null ? LocalDateTime.now() : created);
        e.setDiscountValue(value == null ? BigDecimal.ZERO : value);
        e.setDiscountType(DiscountType.PERCENT);
        return e;
    }
    ...
    void should_FilterCurrentStatus_CodesAndEvents() {
        ...
        var codeCurrent = dc(1, "NEWYEAR", "desc", true, now.minusDays(1), now.plusDays(1), now.minusDays(2),
                new BigDecimal("10"));
        // upcoming: starts > now
        var codeUpcoming = dc(2, "UPCOMING", "desc", true, now.plusDays(1), now.plusDays(5), now.minusDays(1),
                new BigDecimal("5"));
        // inactive
        var codeInactive = dc(3, "INACTIVE", "desc", false, now.minusDays(5), now.plusDays(5), now.minusDays(5),
                new BigDecimal("1"));

        var eventCurrent = ev(10, "Sale Day", "hot", true, now.minusHours(2), now.plusHours(2), now.minusDays(1),
                new BigDecimal("20"));
        var eventUpcoming = ev(11, "Coming Soon", "warm", true, now.plusDays(2), now.plusDays(3), now,
                new BigDecimal("15"));
    ...
    void should_SearchByNameOrDescription_ForCodeAndEvent() {
        ...
        // When (search = "sale" → match codeByDesc.description & eventByName.name;
        // "BLACKFRIDAY" không chứa "sale")
    ...
    void should_SortDescAndPaginate_WithStatusAll() {
        // Given
        LocalDateTime now = LocalDateTime.now();
        // Sort theo startsAt nếu có, otherwise theo createdAt; tất cả included khi
        // status=all
        ...
        // The relative order of items with similar sort keys may vary; assert contents
        // instead of strict order
        assertThat(data2).extracting(m -> m.get("title")).containsExactlyInAnyOrder("A", "C");
