...
public class WorkScheduleGenerationController {
    ...
    public static class GenerateRequest {
        ...
        public List<String> daysOfWeek; // for WEEKLY: tokens like MON,TUE...
    ...
    public ResponseEntity<?> generate(@RequestBody GenerateRequest req) {
        ...
        for (LocalDate d : candidates) {
            ...
            for (com.kopi.kopi.entity.EmployeeShift anchorEs : anchorShifts) {
                ...
                if (shiftId == null)
                    continue;
                // When copying anchor shifts, prefer the anchor's override times if set;
                // otherwise fall back to the shift template times so generated rows
                // always have a scheduled start/end to compare actual check-ins against.
                java.time.LocalTime oStart = anchorEs.getOverrideStartTime();
                java.time.LocalTime oEnd = anchorEs.getOverrideEndTime();
                if (oStart == null && anchorEs.getShift() != null)
                    oStart = anchorEs.getShift().getStartTime();
                if (oEnd == null && anchorEs.getShift() != null)
                    oEnd = anchorEs.getShift().getEndTime();
                ...
                com.kopi.kopi.entity.EmployeeShift newEs = com.kopi.kopi.entity.EmployeeShift.builder()
                        ...
                        .createdAt(LocalDateTime.now())
                        .overrideStartTime(oStart)
                        .overrideEndTime(oEnd)
                        .build();
                if (adminUserId != null)
        ...
        List<String> candStr = new ArrayList<>();
        for (LocalDate d : candidates)
    ...
    public ResponseEntity<?> getWorkScheduleDetails(@PathVariable("id") Integer id) {
        ...
        List<com.kopi.kopi.entity.EmployeeShift> shifts = employeeShiftRepository
    ...
    /**
     * Delete or unlink a single date within a work schedule.
     * - For shifts on dates after today: delete the employee_shifts rows.
     * - For shifts on today or in the past: unlink (set work_schedule_id = NULL).
     */
    @PreAuthorize("hasRole('ADMIN')")
    @org.springframework.web.bind.annotation.DeleteMapping("/apiv1/admin/work-schedules/{id}/date")
    public ResponseEntity<?> deleteWorkScheduleDate(@PathVariable("id") Integer id,
        ...
        LocalDate today = LocalDate.now();
        List<com.kopi.kopi.entity.EmployeeShift> shifts = employeeShiftRepository
                .findByWorkScheduleWorkScheduleIdOrderByShiftDateAsc(id).stream()
                .filter(es -> es.getShiftDate() != null && es.getShiftDate().equals(target)).toList();
