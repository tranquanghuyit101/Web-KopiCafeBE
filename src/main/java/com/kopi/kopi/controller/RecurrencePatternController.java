...
public class RecurrencePatternController {
    ...
    public ResponseEntity<java.util.List<Map<String, Object>>> listPatterns() {
    ...
    public ResponseEntity<Map<String, Object>> updatePattern(@PathVariable Integer id,
        ...
        if (rType == null || rType.isBlank()) {
            return ResponseEntity.badRequest().body(Map.of("message", "recurrenceType required"));
        }
        String type = rType.toUpperCase();
        if ("DAILY".equals(type)) {
            Integer interval = req.getInterval();
            if (interval == null || interval < 1)
                return ResponseEntity.badRequest().body(Map.of("message", "intervalDays must be >= 1"));
            if (interval > 7)
                return ResponseEntity.badRequest().body(Map.of("message", "intervalDays for DAILY cannot exceed 7"));
            ...
            var dup = repo.findAll().stream()
                    ...
                    .findAny();
            if (dup.isPresent())
                return ResponseEntity.status(400).body(Map.of("message", "sample already exists"));

            existing.setRecurrenceType("daily");
            existing.setIntervalDays(interval);
            existing.setDayOfWeek(null);
            repo.save(existing);
            return ResponseEntity.ok(Map.of("message", "Updated", "recurrenceId", existing.getRecurrenceId()));
        } else if ("WEEKLY".equals(type)) {
            java.util.List<String> days = req.getDayOfWeek();
            if (days == null || days.isEmpty())
                return ResponseEntity.badRequest().body(Map.of("message", "dayOfWeek required for WEEKLY"));
            String day = days.get(0);
            String norm = normalizeDayToken(day);
            if (norm == null)
                return ResponseEntity.badRequest().body(Map.of("message", "invalid dayOfWeek"));
        ...
        return ResponseEntity.badRequest().body(Map.of("message", "unsupported recurrenceType"));
    }
    ...
    public ResponseEntity<Map<String, Object>> deletePattern(@PathVariable Integer id) {
        try {
            ...
            return ResponseEntity.status(400)
                    .body(Map.of("message", "Cannot delete pattern due to existing references"));
        }
    }
}
