...
public class EmployeeShiftServiceImpl implements EmployeeShiftService {
    ...
    @Override
    @Transactional
    public Map<String, Object> removeSlots(LocalDate date, Integer shiftId, Integer adminUserId) {
        Map<String, Object> out = new HashMap<>();
        var sopt = shiftRepository.findById(shiftId);
        if (sopt.isEmpty())
            throw new IllegalArgumentException("Shift not found");
    ...
    @Override
    @Transactional
    public Map<String, Object> addEmployeeToShift(LocalDate date, Integer shiftId, Integer employeeId,
        ...
        for (EmployeeShift open : existingForShift) {
            if (open.getEmployee() == null
                ...
                open.setNotes(notes);
                // set override start/end to match the shift template times so actual check-ins
                // can be compared against the scheduled times
                open.setOverrideStartTime(shift.getStartTime());
                open.setOverrideEndTime(shift.getEndTime());
        ...
        EmployeeShift es = EmployeeShift.builder()
                ...
                .status("assigned")
                .notes(notes)
                .overrideStartTime(shift.getStartTime())
                .overrideEndTime(shift.getEndTime())
                .createdAt(LocalDateTime.now())
                .build();
        if (adminUserId != null)
            userRepository.findById(adminUserId).ifPresent(es::setCreatedByUser);
    ...
    public Map<String, Object> removeEmployeeFromShift(Integer employeeShiftId, Integer adminUserId) {
        ...
        LocalDate date = es.getShiftDate();
    ...
    @Override
    @Transactional
    public Map<String, Object> cancelEmployeeShift(Integer employeeShiftId, Integer actingUserId,
        ...
        LocalDate today = LocalDate.now();
        if (es.getShiftDate().isBefore(today))
            throw new IllegalStateException("Cannot cancel past assignment");

        // If the shift is today and currently within active window, do not allow cancel
        LocalDate sd = es.getShiftDate();
        if (sd.equals(today)) {
            java.time.LocalTime now = java.time.LocalTime.now();
            java.time.LocalTime start = es.getOverrideStartTime() != null ? es.getOverrideStartTime()
                    : (es.getShift() != null ? es.getShift().getStartTime() : null);
            java.time.LocalTime end = es.getOverrideEndTime() != null ? es.getOverrideEndTime()
                    : (es.getShift() != null ? es.getShift().getEndTime() : null);
            if (start != null && end != null) {
                if (!now.isBefore(start) && !now.isAfter(end)) {
                    throw new IllegalStateException("Cannot cancel an assignment while the shift is in progress");
                }
            }
        }

        // Only the assigned employee or an admin may cancel
        if (!actingIsAdmin) {
            if (es.getEmployee() == null || es.getEmployee().getUserId() == null
                    || !es.getEmployee().getUserId().equals(actingUserId)) {
                throw new IllegalStateException("Not authorized to cancel this assignment");
            }
        }
        ...
        es.setReason(reason.trim());
        es.setStatus("CANCELLED");
        es.setUpdatedAt(LocalDateTime.now());
        if (actingUserId != null)
            userRepository.findById(actingUserId).ifPresent(es::setUpdatedByUser);
        employeeShiftRepository.save(es);
    ...
    @Override
    @Transactional
    public Map<String, Object> checkinEmployeeShift(Integer employeeShiftId, Integer actingUserId,
        ...
        if (!es.getShiftDate().equals(today))
            throw new IllegalStateException("Can only check in on the day of the shift");
        ...
        java.time.LocalTime end = es.getOverrideEndTime() != null ? es.getOverrideEndTime()
                : (es.getShift() != null ? es.getShift().getEndTime() : null);
        if (start == null || end == null)
            throw new IllegalStateException("Shift start/end time not defined");
        if (now.isBefore(start) || now.isAfter(end))
            throw new IllegalStateException("Check-in only allowed while shift is active");

        // Authorization
        if (!actingIsAdmin) {
            if (es.getEmployee() == null || es.getEmployee().getUserId() == null
                    || !es.getEmployee().getUserId().equals(actingUserId)) {
                throw new IllegalStateException("Not authorized to check in for this assignment");
            }
        }

        if (es.getActualCheckIn() != null)
            throw new IllegalStateException("Already checked in");

        es.setActualCheckIn(LocalDateTime.now());
        es.setUpdatedAt(LocalDateTime.now());
        if (actingUserId != null)
            userRepository.findById(actingUserId).ifPresent(es::setUpdatedByUser);
        employeeShiftRepository.save(es);

        out.put("checkedIn", true);
        out.put("employeeShiftId", employeeShiftId);
        out.put("actualCheckIn", es.getActualCheckIn().toString());
        return out;
    }

    @Override
    @Transactional
    public Map<String, Object> checkoutEmployeeShift(Integer employeeShiftId, Integer actingUserId,
            boolean actingIsAdmin) {
        Map<String, Object> out = new HashMap<>();
        var opt = employeeShiftRepository.findById(employeeShiftId);
        if (opt.isEmpty())
        ...
        // checkout: allow if checked-in (no need for local time vars here)
        if (es.getActualCheckIn() == null)
            throw new IllegalStateException("Cannot check out without a check-in");
        if (es.getActualCheckOut() != null)
            throw new IllegalStateException("Already checked out");

        if (!actingIsAdmin) {
            // Only the assigned employee may check out (admins may force)
            if (es.getEmployee() == null || es.getEmployee().getUserId() == null
                    || !es.getEmployee().getUserId().equals(actingUserId)) {
                throw new IllegalStateException("Not authorized to check out for this assignment");
            }
            // Enforce that non-admin users may only check out after the scheduled end time
            java.time.LocalTime end = es.getOverrideEndTime() != null ? es.getOverrideEndTime()
                    : (es.getShift() != null ? es.getShift().getEndTime() : null);
            if (end == null)
                throw new IllegalStateException("Shift end time not defined");
            java.time.LocalTime now = java.time.LocalTime.now();
            if (!now.isAfter(end))
                throw new IllegalStateException("Check-out only allowed after shift end time");
        }

        es.setActualCheckOut(LocalDateTime.now());
        es.setStatus("COMPLETED");
        // compute overtimeMinutes as difference in minutes between actual_check_out
        // time-of-day and override_end_time time-of-day (may be negative if earlier)
        if (es.getOverrideEndTime() != null && es.getActualCheckOut() != null) {
            ...
            // store the signed difference (user requested formula: actual - scheduled)
            es.setOvertimeMinutes(diff);
        }
        es.setUpdatedAt(LocalDateTime.now());
        if (actingUserId != null)
            userRepository.findById(actingUserId).ifPresent(es::setUpdatedByUser);
        ...
        out.put("checkedOut", true);
        out.put("employeeShiftId", employeeShiftId);
        out.put("actualCheckOut", es.getActualCheckOut().toString());
        return out;
    }

    @Override
    @Transactional
    public Map<String, Object> restoreEmployeeShift(Integer employeeShiftId, Integer actingUserId,
            boolean actingIsAdmin) {
        Map<String, Object> out = new HashMap<>();
        var opt = employeeShiftRepository.findById(employeeShiftId);
        if (opt.isEmpty())
        ...
        // Only allow restore if currently canceled
        String st = es.getStatus() == null ? "" : es.getStatus().toLowerCase();
        if (!(st.contains("cancel")))
            throw new IllegalStateException("Assignment is not canceled");

        // Authorization: only assigned employee or admin
        if (!actingIsAdmin) {
            if (es.getEmployee() == null || es.getEmployee().getUserId() == null
                    || !es.getEmployee().getUserId().equals(actingUserId)) {
                throw new IllegalStateException("Not authorized to restore this assignment");
            }
        }
